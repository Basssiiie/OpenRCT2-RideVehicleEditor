...
test("Set ride type", t =>
{
	...
	setRideType([[ 99, 1]], new RideType(Mock.rideObject({ index: 23 })), 1);
	...
	setRideType([[ 99, 1]], new RideType(Mock.rideObject({ index: 55 })), 1);
...
test("Set seat count", t =>
{
	...
	setSeatCount([[ 58, 1]], 26, 1);
	t.is(car.numSeats, 26);

	setSeatCount([[ 58, 1]], 10, 1);
	t.is(car.numSeats, 10);
});
...
test("Set powered acceleration", t =>
{
	...
	setPoweredAcceleration([[ 42, 1]], 120, 1);
	t.is(car.poweredAcceleration, 120);

	setPoweredAcceleration([[ 42, 1]], 5, 1);
...
test("Set powered max speed", t =>
{
	...
	setPoweredMaximumSpeed([[ 9, 1]], 65, 1);
	t.is(car.poweredMaxSpeed, 65);

	setPoweredMaximumSpeed([[ 9, 1]], 30, 1);
...
test("Set primary colour", t =>
{
	...
	setPrimaryColour([[ 9, 1]], 5, 1);
	t.is(car.colours.body, 5);

	setPrimaryColour([[ 9, 1]], 31, 1);
...
test("Set secondary colour", t =>
{
	...
	setSecondaryColour([[ 19, 1]], 12, 1);
	t.is(car.colours.trim, 12);

	setSecondaryColour([[ 19, 1]], 0, 1);
...
test("Set tertiary colour", t =>
{
	...
	setTertiaryColour([[ 23, 1]], 25, 1);
	t.is(car.colours.tertiary, 25);

	setTertiaryColour([[ 23, 1]], 2, 1);
...
test("Set x position", t =>
{
	...
	setPositionX([[ 29, 1]], 10, 1);
	t.is(car.x, 10);

	setPositionX([[ 29, 1]], 10_090, 1);
	t.is(car.x, 10 + 10_090);

	setPositionX([[ 29, 1]], -3, 1);
...
test("Set y position", t =>
{
	...
	setPositionY([[ 10, 1]], 5020, 1);
	t.is(car.y, 5020);

	setPositionY([[ 10, 1]], -302_010, 1);
	t.is(car.y, 5020 - 302_010);

	setPositionY([[ 10, 1]], 15, 1);
...
test("Set z position", t =>
{
	...
	setPositionZ([[ 101, 1]], -698, 1);
	t.is(car.z, -698);

	setPositionZ([[ 101, 1]], 653, 1);
	t.is(car.z, -698 + 653);

	setPositionZ([[ 101, 1]], -10, 1);
